...
```{r average, fig.width=7, fig.height=3.5, fig.show='hold'}
...
corresp <- arkhe::ca(zcounts1)

## Get row (blue) and column (red) coordinates
rcoords <- arkhe::get_coordinates(corresp, margin = 1)
ccoords <- arkhe::get_coordinates(corresp, margin = 2)

## Plot CA results
ggplot2::ggplot(mapping = ggplot2::aes(x = CA1, y = CA2)) +
  ggplot2::geom_point(data = rcoords, colour = "blue", shape = 6) +
  ggplot2::geom_point(data = ccoords, colour = "red", shape = 15) +
...
## Warning: this may take a few seconds!
(zuni_keep <- bootstrap(corresp, cutoff = fun, n = 1000))

## Plot convex hull
## blue: convex hull for samples; red: convex hull for types
### All bootstrap samples
rchull <- zuni_keep[["row_chull"]]
ggplot2::ggplot(data = rchull, mapping = ggplot2::aes(x = x, y = y, group = id)) +
...
rchull_sub <- subset(rchull, id %in% zuni_keep[["keep"]][[1]])
ggplot2::ggplot(data = rchull_sub, mapping = ggplot2::aes(x = x, y = y, group = id)) +
  ggplot2::geom_polygon(fill = "blue", alpha = 0.1) +
  ggplot2::geom_vline(xintercept = 0, size = 0.5, linetype = "dashed") +
  ggplot2::geom_hline(yintercept = 0, size = 0.5, linetype = "dashed") +
  ggplot2::coord_fixed() + 
...
hull_length <- data.frame(length = zuni_keep[["lengths"]][[1]])
