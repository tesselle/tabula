...
### Usage
```{r richness}
index_richness(mississippi, method = "margalef")
```

```{r asymptotic-richness}
index_composition(mississippi, method = "chao1")
```

```{r rarefaction}
rarefaction(mississippi, sample = 10, method = "hurlbert")
```
...
R <- index_richness(chevelon, method = "count")
R_sim <- simulate(R)
plot(R_sim) 
```
