...
test_that("Brainerd-Robinson", {
  ...
  expect_error(brainerdSimilarity(1:10, 1:5))
  expect_error(brainerdSimilarity(1:26, LETTERS))
})
test_that("Binomial co-occurrence", {
  x <- c(16, 9, 3, 0, 1)
  y <- c(13, 3, 2, 0, 0)
  expect_equal(round(binomialSimilarity(x, y), 2), 0.54)

  expect_error(binomialSimilarity(1:10, 1:5))
  expect_error(binomialSimilarity(1:26, LETTERS))
})
